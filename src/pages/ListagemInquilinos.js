...
function GaugePointer() {
  ...
  return (
    <g>
      ...
      <path d={`M ${cx} ${cy} L ${target.x} ${target.y}`} stroke="var(--primary-gold)" strokeWidth={3} />
    </g>
  );
}
...
function HistoricoComGrafico({ inquilinoId, nome, inquilinoCompleto }) {
  ...
  useEffect(() => {
    if (inquilinoId) {
      fetch(`http://localhost:5500/inquilinos/${inquilinoId}/historico-completo`)
        .then((res) => {
        ...
        .catch((err) => console.error("Erro ao carregar histórico completo:", err));
    }
  }, [inquilinoId]);
  ...
  return (
    <div className="historico-container">
      <div className="historico-gauge">
        ...
        <GaugeContainer width={200} height={200} startAngle={-110} endAngle={110} value={consumoAtual} valueMin={0} valueMax={500}>
          <GaugeReferenceArc />
          <GaugeValueArc />
          <GaugePointer />
        </GaugeContainer>
      ...
      <div className="historico-info">
        ...
        {dadosGrafico.length > 0 ? (
          <BarChart
            ...
            yAxis={[{ scaleType: 'band', dataKey: 'mes', label: 'Mês/Ano', tickLabelStyle: { fill: 'var(--light-text)' } }]}
            xAxis={[{ label: 'Consumo (KWh)', tickLabelStyle: { fill: 'var(--primary-gold)' } }]}
            series={[{ dataKey: 'consumo', label: 'Consumo (KWh)', color: 'var(--primary-gold)' }]}
            layout="horizontal"
            height={300}
            margin={{ left: 80 }}
          />
      ...
      <div ref={relatorioRef} className="relatorio-container-hidden">
        <RelatorioContaDeLuz inquilino={inquilinoCompleto} historicoConsumo={dados} />
      </div>
    </div>
  );
}
...
export default function ListagemInquilinos() {
  ...
  const [forcarAtualizacaoHistorico, setForcarAtualizacaoHistorico] = useState(0);
  ...
  const recarregarInquilinos = async () => {
    try {
      ...
      toast.current?.show({ severity: 'error', summary: 'Erro', detail: 'Falha ao recarregar a lista de inquilinos.' });
    }
  };
  ...
  const handleEditar = (inquilino) => {
    ...
    toast.current.show({ severity: 'info', summary: 'Redirecionando', detail: `Editando ${inquilino.nome}...` });
  };
  ...
  const handleExcluir = async (inquilino) => {
    ...
    try {
      const res = await fetch(`http://localhost:5500/inquilinos/${inquilino.id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
      toast.current.show({ severity: 'success', summary: 'Excluído', detail: `${inquilino.nome} removido.` });
      recarregarInquilinos();
    } catch {
      toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Erro ao excluir inquilino.' });
    }
  };

  const handleAtualizarKwh = async (inquilino) => {
    const novoKwh = prompt(`Novo KWh para ${inquilino.nome}:`, inquilino.kwh_atual || '');
    if (novoKwh !== null && !isNaN(parseFloat(novoKwh))) {
      try {
        const res = await fetch(`http://localhost:5500/inquilinos/${inquilino.id}/atualizar-kwh`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ kwhatual: parseFloat(novoKwh) }),
        });
        if (!res.ok) throw new Error();
        toast.current.show({ severity: 'success', summary: 'Atualizado', detail: 'KWh atualizado com sucesso!' });
        await recarregarInquilinos();
        setForcarAtualizacaoHistorico(prev => prev + 1);
      } catch {
        toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Falha ao atualizar KWh.' });
      }
    }
  };

  const acoesMenu = (inquilino) => [
    { label: 'Editar', icon: 'pi pi-pencil', command: () => handleEditar(inquilino) },
    { label: 'Excluir', icon: 'pi pi-trash', command: () => handleExcluir(inquilino) },
    { label: 'Atualizar KWh', icon: 'pi pi-sync', command: () => handleAtualizarKwh(inquilino) },
  ];
  ...
  return (
    <div className="listagem-page">
      ...
      <div className="listagem-header">
        ...
        <button className="btn-voltar" onClick={() => window.location.href = '/'}>Voltar à Página Inicial</button>
      </div>
              ...
              {inquilinos.length > 0 ? (
                inquilinos.map(i => (
                  <tr key={i.id}>
                    ...
                    <td>{i.numeroap}</td>
                    <td>{i.kwh_atual || '-'}</td>
                    <td>{i.valor_total ? `R$ ${parseFloat(i.valor_total).toFixed(2).replace('.', ',')}` : '-'}</td>
                    <td className="botoes-acoes">
                      <SplitButton label="Opções" model={acoesMenu(i)} className="split-button-custom" />
                      <button className="btn-ver-historico" onClick={() => handleVerHistorico(i)}>
      ...
      {mostrarHistorico && inquilinoSelecionado && (
        <div className="historico-wrapper">
          <button className="btn-fechar-historico" onClick={() => setMostrarHistorico(false)}>
